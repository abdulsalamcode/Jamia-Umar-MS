...
export default function TeacherAttendance() {
  ...
  const fetchAttendance = async () => {
    try {
      setLoading(true)
      setError('')
      const res = await attendanceService.getTeacherAttendance(selectedDate, 
        sectionFilter === 'All' ? null : sectionFilter.toLowerCase()
      )
      setTeachers(res.data.data)

      // Attendance state set karo
      const att = {}
      res.data.data.forEach(t => {
        att[t.id] = t.status || 'present'
      })
      setAttendance(att)
    } catch (err) {
      setError('Failed to load attendance. Please check your server.')
    } finally {
      setLoading(false)
    }
  }

  useEffect(() => { fetchAttendance() }, [selectedDate, sectionFilter])
  ...
  const handleSave = async () => {
    try {
      ...
      await attendanceService.saveTeacherAttendance(selectedDate, attendanceArr)
      setSaved(true)
  ...
  return (
    <div>
      ...
      <div className="mb-4 sm:mb-6">
        <h2 className="text-xl sm:text-2xl font-bold text-green-900">✅ Teacher Attendance</h2>
        <p className="text-xs sm:text-sm text-gray-400 mt-1">
          Track and mark daily attendance for all teaching staff
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
        {[
          { label: 'Total Teachers', value: teachers.length, icon: '👨‍🏫', color: 'bg-green-50 border-green-100'  },
            ...
            {teachers.map(t => (
              <div key={t.id} className="p-4 border-b border-gray-100 last:border-0">
                <div className="flex items-center justify-between">
                  ...
                  <select
                    value={attendance[t.id] || 'present'}
                    onChange={e => handleChange(t.id, e.target.value)}
                    className={`text-xs font-bold px-2 py-1.5 rounded-xl border
                                focus:outline-none cursor-pointer
                      ${attendance[t.id] === 'present' ? 'bg-green-100 text-green-700 border-green-200' :
                        attendance[t.id] === 'absent'  ? 'bg-red-100 text-red-600 border-red-200'       :
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-green-50 border-b border-gray-100">
                  {['#', 'Teacher Name', 'Subject', 'Section', 'Status'].map(h => (
              ...
              <tbody>
                {teachers.map((t, i) => (
                  <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-sm text-gray-500">{t.subject}</td>
                    <td className="px-5 py-3">
